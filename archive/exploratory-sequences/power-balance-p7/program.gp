\\ Power-balance record locations, p = 7: representable N = a^7+b^7+c^7+d^7
...
\\ largest part d and lb(N) is the least m with 4*m^7 >= N.
lista(Nmax) = {
  my(p = 7, K = 1, L = List(), v, best = -1);
  while((K + 1)^p <= Nmax, K++);
  for(a = 0, K,
...
lista(40000000)   \\ bound of the archived b-file; raise for more records
