\\ Power-balance record locations, p = 3: representable N = a^3+b^3+c^3+d^3
...
\\ largest part d and lb(N) is the least m with 4*m^3 >= N.
default(parisize, "128M");   \\ ~510k stored (N,d) pairs at the archived bound
lista(Nmax) = {
  my(p = 3, K = 1, L = List(), v, best = -1);
  while((K + 1)^p <= Nmax, K++);
  for(a = 0, K,
...
lista(300000)   \\ bound of the archived b-file; raise for more records
