\\ Power-balance record locations, p = 4: representable N = a^4+b^4+c^4+d^4
...
\\ largest part d and lb(N) is the least m with 4*m^4 >= N.
lista(Nmax) = {
  my(p = 4, K = 1, L = List(), v, best = -1);
  while((K + 1)^p <= Nmax, K++);
  for(a = 0, K,
...
lista(500000)   \\ bound of the archived b-file; raise for more records
