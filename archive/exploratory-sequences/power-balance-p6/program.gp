\\ Power-balance record locations, p = 6: representable N = a^6+b^6+c^6+d^6
...
\\ largest part d and lb(N) is the least m with 4*m^6 >= N.
lista(Nmax) = {
  my(p = 6, K = 1, L = List(), v, best = -1);
  while((K + 1)^p <= Nmax, K++);
  for(a = 0, K,
...
lista(9000000)   \\ bound of the archived b-file; raise for more records
