; A396760: Positive integers k at which A122921(k) - ceiling(sqrt(k/4)) reaches a new record value.
; Submitted by Nenad Micic
; 1,11,53,96,224,384,896,1536,2816,3584,6144,11264,14336,24576,45056,57344,98304,180224,229376,393216,720896,917504,1572864,2883584,3670016,6291456,11534336,14680064,25165824,46137344,58720256,100663296,184549376,234881024,402653184,738197504,939524096
; Formula: a(n) = b(n-1), b(n) = 4*b(n-3), b(8) = 2816, b(7) = 1536, b(6) = 896, b(5) = 384, b(4) = 224, b(3) = 96, b(2) = 53, b(1) = 11, b(0) = 1
; Observed structure: 1, 11, 53 and the chains 96*4^m, 224*4^m, 2816*4^m merged; matches all 37 verified terms below 10^9 (conjectural beyond).

#offset 1

mov $1,1
mov $2,11
mov $3,53
mov $4,96
mov $5,224
mov $6,384
mov $7,896
mov $8,1536
mov $9,2816
sub $0,1
lpb $0
  rol $1,9
  mov $9,$6
  mul $9,4
  sub $0,1
lpe
mov $0,$1
